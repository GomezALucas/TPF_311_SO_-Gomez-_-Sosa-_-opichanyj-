...
#!/bin/bash
# ==============================================================================
# Punto B - Parte 2: Montaje persistente via /etc/fstab para Rol: manager
# ==============================================================================

# Directorio base del rol manager
ROL_DIR="/home/vagrant/manager"
...
# Obtener UUIDs de las particiones (se obtienen una sola vez)
UUID_SDB1=$(blkid -s UUID -o value /dev/sdc1)
UUID_SDB2=$(blkid -s UUID -o value /dev/sdc2)
UUID_SDB5=$(blkid -s UUID -o value /dev/sdc5)
UUID_SDB6=$(blkid -s UUID -o value /dev/sdc6)
...
# Agregar entradas al fstab
# Usamos un bloque único para evitar duplicados
cat >> /etc/fstab << EOF

# --- Punto B: Montaje TP Final - manager ---
UUID=$UUID_SDB1  $ROL_DIR/Servicios         ext4  defaults  0  2
UUID=$UUID_SDB2  $ROL_DIR/Monitoreo         ext4  defaults  0  2
UUID=$UUID_SDB5  $ROL_DIR/Servicios/Web     ext4  defaults  0  2
UUID=$UUID_SDB6  $ROL_DIR/Servicios/Cache   ext4  defaults  0  2
...
# Verificar estado y guardar output
echo "Montaje completado. Estado actual:"
df -h | grep sdc

# Guardar output para el repo en la carpeta compartida
# Asegúrate de que la ruta /home/vagrant/carpeta_compartida/Punto_B existe
df -h | grep sdc > /home/vagrant/carpeta_compartida/Punto_B/df_output_manager.txt

echo "df_output_manager.txt guardado correctamente."
>>>>>>> Stashed changes
